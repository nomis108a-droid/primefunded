  ...
  return (
    <div className={cn(
      ...
      {isMobile && (
        <div className="p-6 pb-0 flex justify-between items-center">
           <h2 className="text-2xl font-black font-headline tracking-tighter uppercase italic text-white">
             {activeTab === 'positions' ? 'Active Positions' : 'Trade Ledger'}
           </h2>
           <Badge variant="outline" className="text-[10px] font-black uppercase border-primary/20 text-primary">
             {activeTab === 'positions' ? openTrades.length : closedTrades.length} Nodes
           </Badge>
        </div>
      )}
      ...
      <div className={cn(
        ...
        (panelOpen || isMobile) ? "opacity-100" : "opacity-0 pointer-events-none"
      )}>
        {!isMobile ? (
            ...
            {activeTab === 'positions' ? (
              openTrades.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-zinc-600 space-y-4">
                   ...
                   <p className="text-xs font-black uppercase tracking-widest">No active positions</p>
                </div>
              ) : openTrades.map(t => {
                const symbolUpper = t.symbol.toUpperCase();
                const pData = livePrices[symbolUpper];
                ...
                return (
                  <div key={t.id} className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-5 space-y-4 relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-lg font-black text-white italic tracking-tighter uppercase">{t.symbol}</h4>
                        <div className="flex items-center gap-2 mt-0.5">
                           <span className={cn("text-[9px] font-black uppercase px-1.5 py-0.5 rounded", t.type === 'buy' ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500")}>
                             {t.type} {t.lots.toFixed(2)}
                           </span>
                           <span className="text-[9px] font-mono text-zinc-500">#{t.id?.slice(0, 8)}</span>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <p className="text-[9px] font-black uppercase text-zinc-500 tracking-widest mt-1.5">Unrealized P&L</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 border-y border-white/5 py-3">
                       <div>
                         <p className="text-[8px] font-black uppercase text-zinc-500 mb-0.5">Entry Price</p>
                         <p className="text-xs font-bold font-mono text-white">{formatPrice(t.openPrice, t.symbol)}</p>
                       </div>
                       <div className="text-right">
                         <p className="text-[8px] font-black uppercase text-zinc-500 mb-0.5">Current</p>
                         <p className="text-xs font-bold font-mono text-primary">
                           {pData ? (t.type === 'buy' ? pData.bid : pData.ask).toFixed(getPrecision(t.symbol)) : '—'}
                         </p>
                       </div>
                    </div>

                    <div className="flex gap-3">
                      <Button variant="outline" size="sm" className="flex-1 h-11 border-zinc-800 font-bold text-xs rounded-xl" onClick={() => closeTrade(t.id)}>
                        <X size={14} className="mr-2" /> Partial Close
                      </Button>
                      <Button size="sm" className="flex-1 h-11 bg-red-600 hover:bg-red-700 text-white font-black text-xs rounded-xl" onClick={() => closeTrade(t.id)}>
                        EXIT POSITION
                      </Button>
                    </div>
                  </div>
