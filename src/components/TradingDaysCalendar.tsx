...
export function TradingDaysCalendar({ accountId, trades, minTradingDays }: TradingDaysCalendarProps) {
  ...
  const stats = useMemo(() => {
    ...
    return { activeDaysCount: tradeDaysSet.size, tradeDaysSet };
  }, [trades]);
  ...
  const getDayData = (date: Date) => {
    ...
    return { isPast, isActive, isFuture, hasTrade: dayTrades.length > 0, dayPnl };
  };

  if (!hasMounted) {
    return <div className="h-[400px] w-full bg-secondary/10 animate-pulse rounded-3xl" />;
  }
